...
#define PROGRAM_PATH "./pixel_loop_per_8pixel.cl"
...
// openCL parameters
cl_mem gx, gy, gz;
cl_mem gdbase, gdinc, gdlim, gd2s;
cl_mem gcos_orih, gsin_orih, gcos_oriv, gsin_oriv;
cl_mem gwl, ghl, gp2ip;
...
bool my_setup(){
    ...
    localSize = 256;
    globalSize = ceil((PIXEL_COUNT/8)/(float)localSize)*localSize;
    ...
    gd2s = clCreateBuffer(gcontext, CL_MEM_READ_ONLY, sizeof(double), NULL, &err);
    ...
    // screen variables
    gwl = clCreateBuffer(gcontext, CL_MEM_READ_ONLY, sizeof(double), NULL, &err);
    ghl = clCreateBuffer(gcontext, CL_MEM_READ_ONLY, sizeof(double), NULL, &err);
    gp2ip = clCreateBuffer(gcontext, CL_MEM_READ_ONLY, sizeof(double), NULL, &err);
    // map data
    ...
    err = clEnqueueWriteBuffer(gqueue, gd2s, CL_TRUE, 0, sizeof(double), &d2s, 0, NULL, NULL);
    // screen variables
    err = clEnqueueWriteBuffer(gqueue, gwl, CL_TRUE, 0, sizeof(double), &wl, 0, NULL, NULL);
    err = clEnqueueWriteBuffer(gqueue, ghl, CL_TRUE, 0, sizeof(double), &hl, 0, NULL, NULL);
    err = clEnqueueWriteBuffer(gqueue, gp2ip, CL_TRUE, 0, sizeof(double), &p2ip, 0, NULL, NULL);
    // map data
    ...
    err = clSetKernelArg(gkernel,  6, sizeof(cl_mem), (void *)&gd2s);
    err = clSetKernelArg(gkernel,  7, sizeof(cl_mem), (void *)&gcos_orih);
    err = clSetKernelArg(gkernel,  8, sizeof(cl_mem), (void *)&gsin_orih);
    err = clSetKernelArg(gkernel,  9, sizeof(cl_mem), (void *)&gcos_oriv);
    err = clSetKernelArg(gkernel, 10, sizeof(cl_mem), (void *)&gsin_oriv);
    err = clSetKernelArg(gkernel, 11, sizeof(cl_mem), (void *)&gwl);
    err = clSetKernelArg(gkernel, 12, sizeof(cl_mem), (void *)&ghl);
    err = clSetKernelArg(gkernel, 13, sizeof(cl_mem), (void *)&gp2ip);
